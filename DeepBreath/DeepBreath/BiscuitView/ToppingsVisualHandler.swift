...
public class ToppingsVisualHandler: NSObject {
    ...
    public func transition(to: BisState, animated: Bool) {
    ...
    private func fixFrames(for state: BisState, animated: Bool, animateRepositioning: Bool = true) {
        if animated {
        ...
        let block = {
            ...
            if state == .open && self.toppings.count > 0 {
                ...
                self.biscuit.heightConstraint?.constant = BisState.closed.biscuitHeight
            }
        }
    ...
    private func repositionToppings() {
        ...
        var height: CGFloat = BisState.open.titleHeight + self.toppingInsets.top + self.titlePadding
